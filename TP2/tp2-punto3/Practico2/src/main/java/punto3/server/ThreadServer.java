				...
				try {
					...
					switch (decodedMsg.getHeader("tipo")) {
						case "query":
							...
							this.log.info("["+routingKey.substring(8)+"] Cliente ha enviado un msg... \n||" + (new Gson()).toJson(decodedMsg).toString());
							...
							boolean ready = false;
							log.info("["+routingKey.substring(8)+"] Esperando respuesta....");
							while (!ready) {
							...
							log.info("["+routingKey.substring(8)+"] [S->C] Mensaje enviado a cliente... "  + (new Gson()).toJson(resultMsg).toString());
							break;
						case "response":
