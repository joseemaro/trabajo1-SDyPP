			...
			synchronized (dispatcher.getNodosActivos()) {
				...
				if (n != null) {
					if (flag) { //[node finished his task]
						dispatcher.log.info(" [msgProcess -"+queueName+"] Msg notification enviado por ["+ nodeName + "]!");
						// Update Node Load
						n.decreaseCurrentLoad();
						dispatcher.log.info(" [msgProcess -"+queueName+"] currentLoad ["+ nodeName + "]: " + n.getCargaActual());
						...
						dispatcher.log.info(" [msgProcess -"+queueName+"] Msg notification no llego desde ["+ nodeName + "] - TIMEOUT REACHED!");
						// Update Node Load
						n.decreaseCurrentLoad();
						dispatcher.decreaseGlobalMaxLoad(n.getCargaMax());
						synchronized (dispatcher.getNodosActivos()) {
				...
				dispatcher.log.info(" [msgProcess -"+queueName+"] GlobalCurrentLoad [" + dispatcher.getCargarActualGlobal() + "]");
